...
class Pipeline(object):
  ...
  @property
  def cpu_used(self):
    """Returns the amount of CPU this pipeline has used. It returns None
    if the Pipeline has yet to be started, and an integer value otherwise."""
...
def _get_internal_status(pipeline_key=None,
  ...
  Returns:
    Dictionary with the keys:
      ...
      startCpuMegacycles: The CPU time at the beginning of the pipeline.
      endCpuMegacycles: The CPU time at the end of the pipeline.
  ...
  if status in ('run', 'finalizing', 'done', 'retry'):
    if pipeline_record.next_retry_time is not None:
      ...
      output['startCpuMegacycles'] = quota.get_request_cpu_usage()
    elif start_barrier:
      # start_barrier will be missing for root pipelines
  ...
  if status in ('finalizing',):
    ...
    output['endCpuMegacycles'] = quota.get_request_cpu_usage()
