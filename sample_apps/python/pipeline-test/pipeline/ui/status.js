...
function constructStageNode(pipelineId, infoMap, sidebar) {
  ...
  if (infoMap.status == 'done') {
    ...
    statusRunCpuDiv = $('<div class="status-runcpu">');

    var statusCpuSpan = $('<span class="status-cpu-label">');
    statusCpuSpan.text('CPU Used: ');
    statusRunCpuDiv.append(statusCpuSpan);

    var runCpuSpan = $('<span class="status-runcpu-value">');
    runCpuSpan.text(getElapsedCpuString(
        infoMap.startCpuMegacycles, infoMap.endCpuMegacycles));
    statusRunCpuDiv.append(runCpuSpan);
  ...
  // Runcpu if present.
  if (statusRunCpuDiv) {
    containerDiv.append(statusRunCpuDiv);
  }
