...
"""
  ...
  def getLogs(self, powerStr):
    ...
    if resultStr is None:
      ...
      resultStr = users.getMRLogs(outputFile)
...
"""
  ...
  def runMRJob(self,powerStr):
    ...
    if resultStr is None:
      ...
      if authorized == "success":
        jobStarted = users.runMRJob(mapFile, redFile, inputFile, outputFile)

        if jobStarted == "this user cannot call the mapreduce api":
          return

        resultStr = users.getMROutput(outputFile)
        if resultStr == "this user cannot call the mapreduce api":
          return
  ...
  def genInput(self, power, inputLoc):
    n = 2 ** power
    numOfNodes = users.getNumOfNodes()
    ...
    result = users.putMRInput(buckets, inputLoc)
