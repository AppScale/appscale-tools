...
class DescribeCreds(webapp.RequestHandler):
  def get(self):
    user = users.get_current_user()
    cert = str(users.get_ec2_cert()).replace("\n", "<br />")
    pk = str(users.get_ec2_pk()).replace("\n", "<br />")
    ec2_url = str(users.get_ec2_url()).replace("\n", "<br />")
    s3_url = str(users.get_s3_url()).replace("\n", "<br />")
    ec2_access_key = str(users.get_ec2_access_key()).replace("\n", "<br />")
    ec2_secret_key = str(users.get_ec2_secret_key()).replace("\n", "<br />")
